...
let useMacroTask: boolean = false;   // 强制使用 (marco) task 的标记
...
/* istanbul ignore if */
if ('undefined' !== typeof(setImmediate) && isNative(setImmediate)) {
...
export function withMacroTask (fn): () => any {
    ...
    // 设置全局变量禁用 (macro) task
    // 返回结果
    return fn._withTask || (fn._withTask = function (): any {
        useMacroTask = true;
        const res: any = fn.apply(null, arguments);
        useMacroTask = false;
        return res;
    });
}
...
export function nextTick<T>(cb: () => void, ctx?: T): Promise<T> | void {
    ...
    // $flow-disable-line
    if (!cb && 'undefined' !== typeof(Promise)) {
